...
use crate::constant::{
    ...
    POMODORO_SESSION_TABLE, STUDY_STREAK_CURRENT_FIELD, STUDY_STREAK_LAST_DAY_FIELD,
...
impl PomodoroSession {
    ...
    /// Whether this stint moved the lifetime counters, as [`Self::finish`]
    /// judged it. `None` while it is still running — and on any stint closed
    /// before the rule existed, which cannot honestly be given a verdict now.
...
mod tests {
    ...
    async fn a_stint_under_the_minimum_is_logged_and_listed_but_counts_for_nothing() {
        ...
        start_aged(&user, &db, MIN_COUNTED_POMODORO_MS - 60_000)
            .await
            .unwrap();
        let short = PomodoroSession::finish(&user, &db).await.unwrap();
        ...
        let log = PomodoroSession::list_for_user(&user, &db).await.unwrap();
        assert_eq!(log.len(), 1);
        ...
        start_aged(&user, &db, MIN_COUNTED_POMODORO_MS)
            .await
            .unwrap();
        let counted = PomodoroSession::finish(&user, &db).await.unwrap();
        ...
        assert_eq!(
            PomodoroSession::list_for_user(&user, &db)
                .await
                .unwrap()
                .len(),
            2
    ...
    async fn the_day_quota_caps_the_counter_and_rolls_at_midnight_utc() {
        ...
        for _ in 0..3 {
            start_aged(&user, &db, MIN_COUNTED_POMODORO_MS)
                .await
                .unwrap();
            let over = PomodoroSession::finish(&user, &db).await.unwrap();
