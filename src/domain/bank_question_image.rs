...
/// What one [`BankQuestionImage::upsert`] transaction returns: the row it
...
impl BankQuestionImage {
    ...
    /// [`crate::domain::pool_question::bump_question_and_write`] moves
    ...
    /// [`crate::domain::bank_question::BankQuestion::delete`] has removed but
    ...
    /// [`crate::domain::pool_question::PoolQuestion::set_image`] has): two
    ...
    pub async fn upsert(
        ...
        let (question, id) = (self.bank_question.record(), self.id.record());
...
mod tests {
    ...
    async fn upsert_replaces_per_slot() {
        ...
        let (first, retired) = BankQuestionImage::new(&question, None, png(), 3)
            .upsert(&db)
            .await
            .unwrap();
        assert_eq!(retired, None, "a first upload retires no blob");
        let (second, retired) = BankQuestionImage::new(&question, None, png(), 5)
            .upsert(&db)
            .await
            .unwrap();
    ...
    /// The [`crate::domain::pool_question::PoolQuestion`] defect, one domain
    ...
    /// — the read sees a row [`BankQuestion::delete`] has removed but not
    /// committed, while its `DELETE bank_question_image WHERE bank_question =
    /// $b` ran on a snapshot predating this row, so both commit and the image
    /// is left pointing at a template that is gone. Nothing can reach it after
    /// that: every image route resolves the template first, no boot sweep
    /// visits this table, and its blob stays on disk for good. [`Self::upsert`]
    /// moves the template's own `points` instead, so the two transactions touch
    /// one key and the store refuses a side.
    ///
    ...
    async fn an_image_written_inside_a_delete_window_never_outlives_its_template() {
        ...
        for round in 0..4 {
            let question = a_template(&db).await;
            let template = BankQuestion::read(&question, &db).await.unwrap().unwrap();

            let drop_it = {
                let db = db.clone();
                tokio::spawn(async move { template.delete(&db).await })
            };
            ...
            let child = {
                ...
                tokio::spawn(async move {
                    BankQuestionImage::new(&question, None, png(), 3)
                        .upsert(&db)
                        .await
                })
            };
            ...
            if BankQuestion::read(&question, &db).await.unwrap().is_none() {
                swept += 1;
                orphans += BankQuestionImage::list_for_question(&question, &db)
                    .await
                    .unwrap()
                    .len();
        ...
        eprintln!("BankQuestion::delete raced by an upload: {swept}/4 rounds deleted the template");
        assert!(
            swept > 0,
    ...
    /// [`Self::upsert`] reads it inside its own transaction instead, so the
    ...
    async fn two_uploads_to_one_slot_leave_no_blob_unretired() {
        ...
        for round in 0..3 {
            ...
            let (original, _) = BankQuestionImage::new(&question, None, png(), 1)
                .upsert(&db)
                .await
                .unwrap();
            ...
            for size in [2, 3] {
                ...
                writes.push(tokio::spawn(async move {
                    BankQuestionImage::new(&question, None, png(), size)
                        .upsert(&db)
                        .await
                }));
            }
            ...
            let live = BankQuestionImage::read_slot(&question, None, &db)
                .await
                .unwrap()
                .unwrap();
            assert!(
