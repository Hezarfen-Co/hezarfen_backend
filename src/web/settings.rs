...
async fn update_settings(
    ...
    // Wire → workflow mapping, nothing more. The lists are deliberately not
    // validated here: that happens inside the service, in the workflow's own
    // order, and a stored meal-slot name skips the URL-charset rule — a
    // decision only the locked edit can make (`MealSlotDef::try_kept`).
    let patch = service::settings::SettingsPatch {
        exam_kinds: req.exam_kinds.as_ref().map(|kinds| {
            kinds
                .iter()
                .map(|kind| service::settings::ExamKindInput {
                    name: kind.name.clone(),
                    weight: kind.weight,
                })
                .collect()
        }),
        attendance_statuses: req.attendance_statuses.clone(),
        grade_bands: req.grade_bands.as_ref().map(|bands| {
            bands
                .iter()
                .map(|band| service::settings::GradeBandInput {
                    min: band.min,
        ...
        chatbot_history_turns: req.chatbot_history_turns,
        max_chatbot_threads: req.max_chatbot_threads,
        max_chatbot_message_len: req.max_chatbot_message_len,
        meal_slots: req.meal_slots.as_ref().map(|slots| {
            slots
                .iter()
                .map(|slot| service::settings::MealSlotInput {
                    name: slot.name.clone(),
                    serving_minute: slot.serving_minute,
                })
                .collect()
        }),
