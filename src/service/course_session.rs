...
pub async fn materialize(
    ...
    // Owned captures (`Send` rule of `tx_with_retry` closures): the closure may
    // not hold a reference out of the caller's frame — a `&PgPool` or a
    // `&ClassCourse` captured there fails the higher-ranked `Send` check the
    // function's own future must pass, so no route could await it. `PgPool` is
    // a cheap handle clone and the instance is one row.
    let pool = db.clone();
