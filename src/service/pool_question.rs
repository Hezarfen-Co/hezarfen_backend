...
//! swept solutions ride back for the web layer's blob cleanup. The queries
//! and their transactions live in [`crate::db::pool_question`]; the pure
//! entity and newtypes in [`crate::domain::pool_question`].
...
/// caller's orphan to take back off disk).
...
/// means the question is no longer pending.
...
/// Delete the question and cascade its solutions; the removed rows come back
/// so the web layer can take every image blob off disk.
pub async fn delete(
    db: &Database,
    id: &PoolQuestionId,
