//! The `rag_thread` table: one user's RAG threads, listed
//! newest-activity-first, deleted together with every turn in them. The
//! entity and its validated newtypes live in
//! [`crate::domain::rag_thread`]; this module is where their rows are
//! read and written.
//!
//! The cap is the chatbot nest's, read from the settings singleton
//! (`max_chatbot_threads`) — the school sets one ceiling for its AI package —
//! but the seats are this nest's own (`app_user.rag_thread_count`), so a
//! user's RAG threads and their chatbot threads never spend each other's room.
...
/// never set the knob. The knob is shared with the chatbot nest on purpose
/// (one AI package, one ceiling); the *column* is not — `rag_thread_count`
/// counts only these threads, exactly the rows this claim guards.
///
...
pub async fn create_capped(
    ...
    match query_as!(
        ...
        "WITH seat AS (
         ...
         RETURNING id AS \"id: RagThreadId\", owner AS \"owner: UserId\", title AS \"title: RagThreadTitle\", \
                   created_at AS \"created_at: Timestamp\", updated_at AS \"updated_at: Timestamp\"",
        user.uuid(),
        DEFAULT_MAX_CHATBOT_THREADS,
        RagThreadId::generate().uuid(),
...
/// `rag_thread_owner_updated` index exists for.
...
/// How many threads `user` keeps — the `max_chatbot_threads` cap check, read
/// against this nest's own seats.
pub async fn count_for_user(db: &Database, user: &UserId) -> Result<usize, AppError> {
    let row = sqlx::query!(
        "SELECT count(*) AS threads FROM rag_thread WHERE owner = $1",
        user.uuid()
    )
    .fetch_one(db)
    .await?;
...
pub async fn read_for(
    ...
    let thread = query_as!(
        RagThread,
        "SELECT id AS \"id: RagThreadId\", owner AS \"owner: UserId\", title AS \"title: RagThreadTitle\", created_at AS \"created_at: Timestamp\", updated_at AS \"updated_at: Timestamp\" FROM rag_thread WHERE id = $1",
        id.uuid()
    )
    .fetch_optional(db)
    .await?;
    Ok(thread.filter(|thread| &thread.owner == user))
}
...
pub async fn rename(
    ...
    let updated = query_as!(
        RagThread,
        "UPDATE rag_thread SET title = $2, updated_at = GREATEST($3, updated_at + 1) \
         WHERE id = $1 \
         RETURNING id AS \"id: RagThreadId\", owner AS \"owner: UserId\", title AS \"title: RagThreadTitle\", \
                   created_at AS \"created_at: Timestamp\", updated_at AS \"updated_at: Timestamp\"",
        thread.get_id().uuid(),
...
///
/// A turn's insert (see [`crate::db::rag_message`]) writes *through*
...
pub async fn delete(db: &Database, thread: RagThread) -> Result<RagThread, AppError> {
    tx_with_retry(db, false, async move |tx| {
        // Children first, exactly like the chatbot's delete: `rag_message`'s
        // foreign key cascades by itself, but a cascade removes rows the
        // caller cannot see, and this transaction also has to read the parent
        // row back to free its seat.
        sqlx::query!(
            "DELETE FROM rag_message WHERE thread = $1",
            thread.get_id().uuid()
        )
        .execute(&mut *tx)
        .await?;
        let gone = query_as!(
            RagThread,
            "DELETE FROM rag_thread WHERE id = $1 \
             RETURNING id AS \"id: RagThreadId\", owner AS \"owner: UserId\", title AS \"title: RagThreadTitle\", \
                       created_at AS \"created_at: Timestamp\", updated_at AS \"updated_at: Timestamp\"",
            thread.get_id().uuid()
        )
        ...
        if let Some(gone) = &gone {
            sqlx::query!(
                "UPDATE app_user SET rag_thread_count = GREATEST(rag_thread_count - 1, 0) \
                 WHERE id = $1",
                gone.get_owner().uuid()
            )
            .execute(&mut *tx)
...
mod tests {
    ...
    use crate::domain::rag_message::RagContent;
    ...
    async fn stored(db: &Database) -> (i64, usize) {
        let counter = sqlx::query("SELECT rag_thread_count FROM app_user WHERE id = $1")
            ...
            .try_get::<i64, _>(0)
            .unwrap();
        let rows = sqlx::query("SELECT count(*) FROM rag_thread")
            .fetch_one(db)
            .await
            .unwrap()
            .try_get::<i64, _>(0)
    ...
    async fn a_turn_writes_its_thread_and_dies_with_it() {
        ...
        let ask = || RagContent::try_new("selam").unwrap();
        rag_message::append_user(&db, thread.get_id(), &user, ask())
            .await
            .expect("append");
        ...
        let orphan = rag_message::append_user(&db, &id, &user, ask()).await;
        assert!(
            matches!(orphan, Err(AppError::NotFound)),
            "a turn on a deleted thread must be refused: {orphan:?}"
        );
        assert_eq!(
            rag_message::list_for_thread(&db, &id, None, 0)
                .await
                .expect("list")
                .1,
            0,
    ...
    /// **ahead of the clock**, which is precisely what `GREATEST` leaves
    /// behind (see [`crate::db::rag_message`]'s insert) and therefore an
    /// ordinary row, not a contrived one. From there the two rules are told
    /// apart with no race at all — upwards keeps climbing off the stored
    /// value, the clock drags the stamp back down to itself. The lead is a
    /// second, far more than two writes can burn, and the guard below fails
    /// loudly rather than vacuously on a machine that manages to burn it.
    #[tokio::test]
    async fn a_stamp_ahead_of_the_clock_still_climbs() {
        ...
        sqlx::query("UPDATE rag_thread SET updated_at = $1 WHERE id = $2")
            .bind(parked)
        ...
        for _ in 0..2 {
            rag_message::append_user(
                &db,
                thread.get_id(),
                &user,
                RagContent::try_new("selam").unwrap(),
            )
            .await
            .expect("append");
        }
    ...
    /// litter: nothing sweeps `rag_message` afterwards, and a `GET` on the
    /// turn would answer with the text of a thread the user had deleted.
    ///
    ...
    /// row ([`crate::db::rag_message`]'s insert) instead of reading it — the
    /// two transactions then touch one key and the store refuses to commit
    /// both.
    ///
    /// The window the schema event used to force open is the thread row's own
    /// lock now: a turn is written *through* the row, so a turn racing the
    /// delete serializes on that row instead of committing past the sweep.
    /// Both orders are forced by awaiting one side to completion before the
    /// other starts — a barrier is a coin toss under load, and a run where
    /// the delete loses every overlapped round has still proved the invariant.
    /// A few rounds still start together; those may all lose the delete, and
    ...
    async fn a_turn_written_inside_a_delete_never_outlives_the_thread() {
        ...
        async fn append(db: &Database, id: &RagThreadId, user: &UserId) -> Result<(), AppError> {
            rag_message::append_user(db, id, user, RagContent::try_new("selam").unwrap())
                .await
                .map(|_| ())
        }
        ...
        eprintln!(
            "rag_thread::delete raced by a turn: {swept}/4 concurrent rounds deleted the thread"
        );
    }
}
