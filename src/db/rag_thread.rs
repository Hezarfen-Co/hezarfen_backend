...
mod tests {
    ...
    /// delete serializes on that row instead of committing past the sweep. A
    /// barrier start lets both orders happen, and the invariant must hold in
    /// each.
    ...
    async fn a_turn_written_inside_a_delete_never_outlives_the_thread() {
        ...
        for round in 0..8 {
            ...
            if read_for(&db, &id, &user).await.unwrap().is_none() {
                swept += 1;
                orphans += rag_message::list_for_thread(&db, &id, None, 0)
                    .await
                    .unwrap()
                    .1;
        ...
        eprintln!("rag_thread::delete raced by a turn: {swept}/8 rounds deleted the thread");
        assert!(
            swept > 0,
