...
#[openapi(
    ...
    tags(
        ...
        (name = "classes", description = "Classes (şube): a named set of students the school moves as one. A class is bulk enrollment, not a second kind of membership — adding a student enrolls them into every course the class carries, attaching a course enrolls the whole roster into it, and the rows written are ordinary enrollments. A course with too few seats takes none of the class (409, naming the course), a student already enrolled by hand keeps their own row (never adopted, never swept), and a class still holding students or courses refuses deletion (409). The class itself and its roster are manager+; attaching or detaching a course takes management rights on *that course*, since it writes that course's roster"),
