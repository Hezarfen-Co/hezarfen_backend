...
#[openapi(
    ...
    tags(
        ...
        (name = "bank", description = "The school-wide question bank: a shared library of reusable question templates. Any teacher+ reads the whole bank (`GET /bank-questions`, optional `?subject=` origin filter) and instantiates any template into their own exam (`POST /exams/{id}/questions/from-bank/{bid}`, which copies the template — text, points, `choice`/`text` spec, illustration, and option pictures — into a fresh exam question under the target subject, source untouched); a teacher also saves an existing exam question back into the bank (`POST /exams/{id}/questions/{qid}/to-bank`). A template's `subject` is origin metadata only — the bank spans courses, so the same-course rule is checked at instantiate time against the target exam's course, never on the bank row. Reads and instantiate are school-wide, but editing or deleting a template needs ownership (its owner, or an admin — 403 otherwise); templates never freeze, since they carry no exam tie. Images mirror exam question images (one illustration, one picture per option on `choice` templates; raster types only, ≤ the school's `max_file_bytes`), owner-gated to write and school-wide to read"),
