...
#[openapi(
    ...
    tags(
        ...
        (name = "holidays", description = "The school-wide non-teaching calendar. A holiday is one named instant range (`starts_at`/`ends_at`, UTC unix-milliseconds, like every other dated resource here) and nothing else: no class, no course, no per-role audience — the whole school is out. `kind` is one of the fixed set (`resmi` official, `dini` religious, `idari` administrative, `ara` a break; `GET /limits` publishes them). Manager+ declares, edits and deletes; every authenticated user reads, because students need the calendar to make sense of their courses and their lessons. `GET /holidays?from=&to=` windows the list with **overlap** semantics — a holiday that merely reaches into the window is listed. The calendar's one workflow consumer is the weekly-plan materializer (`POST /instances/{id}/weekly-plan/materialize`): a day a holiday reaches into generates no lesson, and the dry run names it in `blocked`."),
    ),
)]
struct ApiDoc;
...
pub fn build_router(state: AppState) -> Router {
    let (router, api) = OpenApiRouter::with_openapi(ApiDoc::openapi())
        ...
        .merge(web::school::routes())
        .nest("/ai", web::ai::routes())
        .nest("/auth", web::auth::routes(&state))
        .merge(web::builder::routes(&state))
        // The insights directory is the one route of that nest a school
        // session may not reach (it names every customer), so it is mounted
        // here on the builder principal instead of inside the gate below.
        .merge(web::insights::school_directory_routes())
        .nest(
            "/chatbot",
        ...
        .nest("/holidays", web::holidays::routes())
        .nest(
            "/boards",
...
/// Count the request in and out and record its duration. The school uuid is
